...
Recording which licenses the dependencies use is enough for the source releases:
publishing a crate to crates.io distributes that crate's source and nothing else.
The `pyiceberg-core` wheels are different. They ship a compiled extension module
that statically links every Rust dependency, so distributing a wheel redistributes
those dependencies and triggers the attribution conditions of their licenses --
which call for the full license text, not just an identifier.
