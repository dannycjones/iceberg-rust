...
def main():
    ...
    parser.add_argument(
        "branch",
        help="Release branch name (e.g. 0.10.x)",
    )
    parser.add_argument(
        "--repo",
    ...
    branch_match = re.fullmatch(r"(\d+)\.(\d+)\.x", args.branch)
    if not branch_match:
        parser.error(f"Branch '{args.branch}' does not match expected pattern '<major>.<minor>.x'")
